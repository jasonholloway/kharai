...
type SetHandlersFn<A> = (resolve: (v:A|Promise<A>)=>void, reject: (r:any)=>void, onCancel: (h:Hook)=>void) => void;

export default class CancellablePromise<A> implements PromiseLike<A>, Cancellable {

  private readonly _spawn: <B>(fn: SetHandlersFn<B>) => CancellablePromise<B>;
  private readonly _cancel: () => Promise<void>;
  private readonly _inner: Promise<A>;

  constructor(setHandlers: SetHandlersFn<A>, hooks: Hook[] = []) {
